...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::PROX, nValue);
...
{
    std::string genesisblockhash = "0000015f05983151d459f07061a6025225705e04f28bf76506b4841d1949caa5";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (prox.conf).");
            QMessageBox::warning(this, "Proximity Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
