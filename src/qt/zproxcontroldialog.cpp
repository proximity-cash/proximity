...
#include "ui_zproxcontroldialog.h"
...
std::list<std::string> ZproxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZproxControlDialog::listMints;

ZproxControlDialog::ZproxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZproxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZproxControlDialog::~ZproxControlDialog()
{
    delete ui;
}

void ZproxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZproxControlDialog::updateList()
{
...
void ZproxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZproxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZprox_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZproxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZproxControlDialog::GetSelectedMints()
{
...
void ZproxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
