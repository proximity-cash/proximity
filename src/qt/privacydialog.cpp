                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zPROX ought to be enough for anybody." - Bill Gates, 2017
    ui->zPROXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzPROXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzPROX->setEnabled(false);
        ui->pushButtonMintzPROX->setToolTip(tr("zPROX is currently disabled due to maintenance."));

        ui->pushButtonSpendzPROX->setEnabled(false);
        ui->pushButtonSpendzPROX->setToolTip(tr("zPROX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zPROXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzPROX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPROX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zPROX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zPROX in ") +
...
void PrivacyDialog::on_pushButtonSpendzPROX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPROX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zPROX
        sendzPROX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zPROX
    sendzPROX();
}

void PrivacyDialog::on_pushButtonZproxControl_clicked()
{
    ZproxControlDialog* zproxControl = new ZproxControlDialog(this);
    zproxControl->setModel(walletModel);
    zproxControl->exec();
}

void PrivacyDialog::setZproxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzproxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzPROX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid prox Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zPROXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zPROX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " PROX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zPROX</b>";
    ...
    // use mints from zprox selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZproxControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZproxControlDialog::GetSelectedMints();
    }
    ...
    // Clear zprox selector in case it was used
    ZproxControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zprox Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " prox, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zprox Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zPROX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zPROX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zPROX "));
    ui->labelzPROXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzPROXSyncStatus->setVisible(fShow);
}
